...
class Route2 extends Data {
    ...
    private function setCurrent($current=null): void
    {
    ...
    private static function route_select_info($object, $record): stdClass
    {
        ...
        $select->host = [];
        $select->attribute[] = Route2::SELECT_DEFAULT;
        $key = 0;
        $select->parameter->{$key} =  Route2::SELECT_DEFAULT;
    ...
    public static function wildcard(App $object){
        if(defined('IS_CLI')){
            ...
            $request = Route2::selectWildcard($object, $select);
            $route =  $object->data(App::ROUTE);
    ...
    public static function request(App $object){
        if(defined('IS_CLI')){
            ...
            $select->method = Handler::method();
            $select->host = [];
            $request = Route2::route_select_cli($object, $select);
            if($request === false){
                $select = Route2::route_select_info($object, $select);
                $request = Route2::route_select_cli($object, $select);
            }
            if($request === false){
            ...
            $route =  $object->data(App::ROUTE);
            $object = Route2::add_request($object, $request);
            return $route->current($request);
        } else {
            if(
            ...
            $select = new stdClass();
            $select->input = $input;
            $test = Route2::request_explode(urldecode($input->data('request')));
...
//                $select->deep = substr_count($string_count, '/');
            ...
            $select->host = array_unique($select->host);
            $request = Route2::route_select($object, $select);
            $route =  $object->data(App::ROUTE);
            Route2::add_request($object, $request);
            return $route->current($request);
        }
    }
    ...
    private static function route_select_cli($object, $select){
        ...
        foreach($data as $name => $record){
            ...
            $match = Route2::is_match_cli($object, $record, $select);
            if($match === true){
                $current = $record;
                $current->name = $name;
                break;
        ...
        if($current !== false){
            $current = Route2::prepare($object, $current, $select);
    ...
    private static function selectWildcard($object, $select){
        ...
        foreach($data as $record){
            ...
            $match = Route2::is_match_by_wildcard($object, $record, $select);
            if($match === true){
                $current = $record;
                break;
            }
        ...
        if($match === false){
            foreach($data as $record){
                ...
                $match = Route2::is_match_by_wildcard_has_slash_in_attribute($object, $record, $select);
                if($match === true){
                    $current = $record;
                    break;
                }
        ...
        if($current !== false){
            if(property_exists($current, 'controller')){
                $current = Route2::controller($current);
            }
            Route2::add_request($object, $current);
            return $current;
        }
        return false;
    }
    ...
    private static function route_select($object, $select){
        ...
        foreach($data as $name => $record){
            ...
            $match = Route2::is_match($object, $record, $select);
            if($match === true){
                $current = $record;
                $current->name = $name;
                break;
        ...
        if($match === false){
            foreach($data as $name => $record){
                ...
                $match = Route2::is_match_has_slash_in_attribute($object, $record, $select);
                if($match === true){
                    $current = $record;
                    $current->name = $name;
                    break;
    ...
    private static function prepare($object, $route, $select){
        ...
        foreach($explode as $nr => $part){
            if(Route2::is_variable($part)){
                $get_attribute = Route2::get_variable($part);
    ...
    private static function is_match_by_attribute($object, $route, $select): bool
    {
        ...
        foreach($explode as $nr => $part){
            if(Route2::is_variable($part)){
                $variable = Route2::get_variable($part);
                if($variable){
    ...
    {
        $is_match = Route2::is_match_by_attribute($object, $route, $select);
        if($is_match === false){
            return $is_match;
        }
        $is_match = Route2::is_match_by_method($object, $route, $select);
        if($is_match === false){
            return $is_match;
        }
        return $is_match;
    ...
    private static function is_match($object, $route, $select): bool
    {
        ...
        $route = Route2::add_localhost($object, $route);
        $is_match = Route2::is_match_by_host($object, $route, $select);
        if($is_match === false){
            return $is_match;
        }
        $is_match = Route2::is_match_by_deep($object, $route, $select);
        if($is_match === false){
            return $is_match;
        }
        ...
        $is_match = Route2::is_match_by_condition($object, $route, $select);
        if($is_match === false){
            return $is_match;
        }
        return $is_match;
    ...
    public static function configure(App $object): void
    {
        ...
        $cache = Route2::cache_read($object, $url, $cache_url);
        $cache = Route2::cache_invalidate($object, $cache);
        if(empty($cache)){
            if(File::exist($url)){
                $read = File::read($url);
                $data = new Route2(Core::object($read));
                $data->url($url);
                $data->cache_url($cache_url);
                $object->data(App::ROUTE, $data);
                Route2::load($object);
                Route2::framework($object);
                Route2::cache_write($object);
            } else {
                $data = new Route2();
                $data->url($url);
                $data->cache_url($cache_url);
    ...
    private static function cache_invalidate($object, $cache)
    {
        ...
        elseif($has_resource === false) {
            ...
            Route2::cache_mtime($object, $cache);
            return $cache;
        }
    }
     ...
     * @throws ObjectException
     */
    private static function cache_read($object, $url, $cache_url): ?Route2
    {
        if(File::Exist($cache_url)){
    ...
    public static function load($object){
        ...
        foreach($data as $item){
            ...
            if(!property_exists($item, 'resource')){
                $item = Route2::item_path($object, $item);
                $item = Route2::item_deep($object, $item);
                continue;
            }
            if(property_exists($item, 'read')){
                continue;
            }
            $item->resource = Route2::parse($object, $item->resource);
    ...
    public static function parse($object, $resource){
        ...
        if(isset($temp[1])){
            ...
            return Route2::parse($object, $resource);
        } else {
            return $resource;
        }
    }
